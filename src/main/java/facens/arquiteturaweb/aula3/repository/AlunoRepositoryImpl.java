...
public class AlunoRepositoryImpl implements AlunoRepository {
    ...
    public List<Aluno> findAll() {
        ...
        return jdbcTemplate.query("SELECT * FROM aluno", (resultSet, rowNum) -> {
            ...
            return new Aluno(
                    ...
                    resultSet.getString("sexualidade"),
    ...
    public Aluno save(facens.arquiteturaweb.aula3.modelo.Aluno aluno) {
        if (aluno.getId() != null) {
            String insertQuery = "INSERT INTO public.aluno (id, nome, curso, nascimento, sexualidade, sexo) VALUES (?, ?, ?, ?, ?, ?)";
            /*
             ...
             */
            jdbcTemplate.update(insertQuery, aluno.getId(), aluno.getNome(), aluno.getCurso(), aluno.getNascimento(),
                    aluno.getSexualidade(), aluno.getSexo());
        } else {
            String updateQuery = "UPDATE public.aluno SET nome = ?, curso = ?, nascimento = ?, sexualidade = ?, sexo = ? WHERE id = ?";
            jdbcTemplate.update(updateQuery, aluno.getNome(), aluno.getCurso(), aluno.getNascimento(), aluno.getSexualidade(),
                    aluno.getSexo(), aluno.getId());
        }
        return aluno;
    }
}
